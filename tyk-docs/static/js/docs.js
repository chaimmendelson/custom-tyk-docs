...
$(document).ready(function(e){
	$.fn.copyToClipboard = function () {
		return this.each(function () {
			...
			var $image = $('<img src="/img/copy.png" alt="Copy code" class="copy-icon">');
			...
			var copyTextToClipboard = function () {
				try {
					...
					$image.attr('src', '/img/copy.png').prop('disabled', true);
				}
			...
			var showCopiedLayout = function () {
				$text.text('Copied');
				$image.attr('src', '/img/check.png'); // Change the image to a tick
			};

			var resetLayout = function () {
				$text.text('');
				$image.attr('src', '/img/copy.png'); // Change the image back to 'Copy'
...
//Handle header hyperlinks
	$('.wysiwyg').find('h2:not(.see_also_heading), h3:not(.see_also_heading), h4:not(.see_also_heading), h5:not(.see_also_heading)').hover(function () {
		$(this).append('<a href=#' + $(this).context.id + '><img src="/img/link.svg" />  </a>'); },
		function(){
